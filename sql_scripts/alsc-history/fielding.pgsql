-- drop view fielding_01_p1_career_dismissals;
CREATE OR REPLACE VIEW fielding_01_p1_career_dismissals AS
SELECT 
    z_player_matches_all.Name as "Name"
    , Sum(CASE WHEN lower(wickets.how_out) in ('caught','stumped','run out') then 1 else 0 end) AS "Dismissals"
    ...
    , Sum(CASE WHEN lower(wickets.how_out) in ('run out') then 1 else 0 end) AS "Run Outs"
...
-- drop view fielding_02_p1_season_dismissals;
...
-- drop view fielding_03_p1_innings_dismissals;
...
-- drop view fielding_04_p1_ct_b_combos;
...
WHERE lower(wickets.how_out) in ('caught','stumped')
GROUP BY "Fielder", "Bowler", Wickets.assist, Wickets.playerID
HAVING (Wickets.assist) Is Not Null
ORDER BY Count(Wickets.how_out) DESC, "Catches", "Bowler";
