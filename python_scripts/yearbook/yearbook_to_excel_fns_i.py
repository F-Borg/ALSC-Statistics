...
def yb_milestones(_season_, writer, wb, pgconn):
    ...
    SELECT
        "Name",
        ("Total Matches" - mod("Total Matches"::int,50))::varchar || ' Matches: ' || "Name" as "Milestone",
        "Season Matches",
        "Total Matches",
        playerid
    FROM (
    ...
    SELECT
        "Name",
        ("Total Runs" - mod("Total Runs"::int,500))::varchar || ' Runs: ' || "Name" as "Milestone",
        "Season Runs",
        "Total Runs",
        playerid
    FROM (
    ...
    SELECT
        "Name",
        ("Total Wickets" - mod("Total Wickets"::int,50))::varchar || ' Wickets: ' || "Name" as "Milestone",
        "Season Wickets",
        "Total Wickets",
        playerid
    FROM (
